...
local math       = math
local ipairs     = ipairs
local unpack     = unpack or
table.unpack                          -- luacheck: globals unpack (compatibility with Lua 5.1)
...
local function prompt_text_with_cursor(args)
    ...
    elseif #text < args.cursor_pos then
        ...
        char = gstring.xml_escape(text:sub(args.cursor_pos,
            args.cursor_pos + offset))
        spacer = " "
...
function prompt:start()
    ...
    self._private.grabber = keygrabber.run(function(modifiers, key, event)
        ...
        if mod.Control then
            ...
            elseif key == "p" then
                if history_index > 1 then
                    ...
                    self.command = data.history[self.history_path].table
                    [history_index]
            ...
            elseif key == "r" then
                self._private.search_term = self._private.search_term or
            ...
            elseif key == "h" then
                if self._private_cur_pos > 1 then
                    ...
                    self.command = self.command:sub(1,
                        self._private_cur_pos - 2 - offset) ..
            ...
            elseif key == "k" then
                self.command = self.command:sub(1, self._private_cur_pos - 1)
            elseif key == "u" then
                self.command = self.command:sub(self._private_cur_pos,
                    #self.command)
                self._private_cur_pos = 1
            elseif key == "Prior" then
                self._private.search_term = self.command:sub(1,
                    self._private_cur_pos - 1) or ""
                for i, v in (function(a, i) return itera(-1, a, i) end), data.history[self.history_path].table, history_index do
            ...
            elseif key == "w" or key == "BackSpace" then
                ...
                self.command = self.command:sub(1, cword_start_pos - 1) ..
                self.command:sub(cword_end_pos + 1)
            ...
            elseif key == "Delete" then
                ...
                if self.command == data.history[self.history_path].table[history_index] then
                    table.remove(data.history[self.history_path].table,
                        history_index)
                    if history_index <= history_items(self.history_path) then
                        self.command = data.history[self.history_path].table
                        [history_index]
        ...
        elseif mod.Mod1 or mod.Mod3 then
            if key == "b" then
                self._private_cur_pos = cword_start(self.command,
                    self._private_cur_pos)
            elseif key == "f" then
                self._private_cur_pos = cword_end(self.command,
                    self._private_cur_pos)
            elseif key == "d" then
                self.command = self.command:sub(1, self._private_cur_pos - 1) ..
            ...
            elseif key == "BackSpace" then
                ...
                self.command = self.command:sub(1, wstart - 1) ..
                self.command:sub(self._private_cur_pos)
            ...
            if self.completion_callback then
                if key == "Tab" or key == "ISO_Left_Tab" then
                    ...
                    local matches
                    self.command, self._private_cur_pos, matches = self
                    .completion_callback(command_before_comp, cur_pos_before_comp,
                        ncomp)
                    ncomp = ncomp + 1
                    key = ""
...
local function new(args)
    ...
    args.font                = args.font or beautiful.prompt_font or
    beautiful.font
    args.bg_cursor           = args.bg_cursor or beautiful.prompt_bg_cursor or
    beautiful.bg_focus or "white"
    args.fg_cursor           = args.fg_cursor or beautiful.prompt_fg_cursor or
    beautiful.fg_focus or "black"
    args.ul_cursor           = args.ul_cursor or nil
    args.reset_on_stop       = args.reset_on_stop == nil and true or
    args.reset_on_stop
