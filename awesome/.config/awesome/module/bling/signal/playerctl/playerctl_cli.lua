...
function playerctl:set_loop_status(loop_status, player)
    if player ~= nil then
        awful.spawn.with_shell("playerctl --player=" ..
        player .. " loop " .. loop_status)
    else
...
function playerctl:cycle_loop_status(player)
    ...
    if player ~= nil then
        ...
        "playerctl --player=" .. player .. " loop", function(stdout)
...
function playerctl:cycle_shuffle(player)
    if player ~= nil then
        awful.spawn.easy_async_with_shell(
        "playerctl --player=" .. player .. " shuffle", function(stdout)
                ...
                callback = function()
                    if title and title ~= "" then
                        if art_url ~= "" then
                            ...
                            helpers.filesystem.save_image_async_curl(art_url,
                                art_path, function()
                                self:emit_signal("metadata", title, artist,
                                    art_path, album, player_name)
                            ...
                            self:emit_signal("metadata", title, artist, "", album,
                                player_name)
                            capi.awesome.emit_signal(
                            "bling::playerctl::title_artist_album", title, artist,
                                "")
                        end
                    else
        ...
        awful.spawn.easy_async_with_shell(length_cmd, function(length)
            ...
            if length_sec and interval_sec then
                if interval_sec >= 0 and length_sec > 0 then
                    self:emit_signal("position", interval_sec,
                        length_sec / 1000000)
                    capi.awesome.emit_signal("bling::playerctl::position",
