...
local function timed(args)
	...
	function obj:reset_values()
		...
		self.intro = args.intro or
		(RUBATO_MANAGER.timed.defaults.intro > self.duration * 0.5 and self.duration * 0.5 or RUBATO_MANAGER.timed.defaults.intro)
		...
		assert(self.intro + self.outro <= self.duration or self.prop_intro,
			"Intro and Outro must be less than or equal to total duration")
		assert(self.intro + self.outro <= 1 or not self.prop_intro,
	...
	local function set(value)
		...
		if obj.is_instant then
			obj:fire(preproceesvalue, obj.duration, obj.pos - value); return
		end
		...
		if not RUBATO_TIMEOUTS[obj._dt_index][obj.rate] then RUBATO_TIMEOUTS[obj._dt_index][obj.rate] = create_timeout(
			obj.rate, obj.override_dt) end
		obj._dt = 1 / obj.rate
	...
	function obj:fire(...)
		args = ({ ... })[1] and { ... } or { obj.pos, obj._time, obj._dx }; for _, func in pairs(obj._subscribed) do func(
			unpack(args)) end
	end
